...
The caret package in R was used to quickly run a number of different types of models on the same data.
...
The resamples function was used to aggregate the results from the models.
```{r, echo=TRUE}
results <- resamples(list(logistic=fit.glm, cart=fit.cart, c50=fit.c50,
                          bagging=fit.treebag, rf=fit.rf, gbm=fit.gbm))
```

The method accuracy was then compared across models.
```{r, echo=TRUE}
summary(results, metric="Accuracy")
```

The same results are shown in a boxplot and dotplot.
```{r, echo=TRUE}
...
The logistic regression model showed the highest accuracy so it is summarized further to understand making the greatest contribution to the model.
```{r, echo=TRUE}
summary(fit.glm)
```

Another logistic regression model was attempted using more simple 
